...
def sum_and_compare(arr: list[int]) -> str:
    arr_sum = sum(arr)
    max_num = max(arr)
    difference = arr_sum - max_num

    odd_or_even = "even" if difference % 2 == 0 else "odd"
    return f"{arr_sum} - {max_num} = {difference} is {odd_or_even}"
    
# 15 > 5   =>   15 - 5 = 10 is even | odd
