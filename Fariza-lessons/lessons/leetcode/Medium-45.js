...
var jump = function (nums) {
    ...
    let currentPlace = 0; // maximum reachable index from the current position

    for (let i = 0; i < nums.length - 1; ++i) { // Iterate through the array
        currentPlace = Math.max(currentPlace, i + nums[i]); // Update the maximum reachable index from the current position
        ...
        if (i === curEnd) { // If the current position is equal to the current maximum reachable index
            ...
            curEnd = currentPlace; // Update the current maximum reachable index
        }
    }
    return answer; // Return the minimum number of jumps needed

    // steps = [2, 3, 1, 1, 4]
    // currentPlace == 0 && 0+2 > 0 => currentPlace = 2      //          0===0  => answer = 1, curEnd = 2
    // currentPlace == 2 && 1+3 > 2 => currentPlace = 4      //          1===2  => answer = 1, curEnd = 2
    // currentPlace == 4 && 2+1 > 4 => currentPlace = 4      //          2===2  => answer = 2, curEnd = 4
    // currentPlace == 4 && 3+1 > 4 => currentPlace = 4      //          3===4  => answer = 2, curEnd = 4
    // currentPlace == 4 && 4+4 > 4 => currentPlace = 8      //          4===8  => answer = 2, curEnd = 4
    // return 2
}
