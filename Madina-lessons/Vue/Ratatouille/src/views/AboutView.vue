...
async function deleteDishFn(dishID) {
  let confirmation = confirm("Are you sure?")
  if (confirmation) {
    await store.dispatch('deleteDish', dishID)
    alert("SUCCESS!")
  }
  await fetchDishes()
}
...
<template>
  <div class="about-page-wrapper">
    ...
    <div class="box">
      ...
      <button @click="(e) => { createMode = !createMode; editMode = false }"
        ...
        <div v-if="dishes.length > 0">
          <ul>
            <li v-for="dish in dishes" :key="dish.id">
              ...
              <button class="delete-btn" @click="deleteDishFn(dish.id)">✖</button>
              <h3>{{ dish.name }}</h3>
