...
function removeDuplicates(nums) {
    // SOLUTION WITH ALGORITHM (TWO POINTERS)
    let i = 0;
    for (let n of nums) {
        if (i < 2 || n > nums[i - 2]) {
            nums[i++] = n;
    ...
    return i;
    // [0, 0, 1, 1, 1, 1, 2, 3, 3]   ->  i < 2  True  =>  zero index turns to 0   ->  i = 1
    // [0, 0, 1, 1, 1, 1, 2, 3, 3]   ->  i < 2  True  =>  first index turns to 0  ->  i = 2
    // [0, 0, 1, 1, 1, 1, 2, 3, 3]   ->  i < 2  False || 1 > 0  True  =>  second index turns to 1  ->  i = 3
    // [0, 0, 1, 1, 1, 1, 2, 3, 3]   ->  i < 2  False || 0 > 1  False  ->  continue
    ...
    // for (let n of nums) {
    //     if (!pool.includes(n)) {
    //         pool.push(n) // first time adding the number
    //     } else if (pool.includes(n) && pool.indexOf(n) === pool.lastIndexOf(n)) {
    //         pool.push(n) // second time adding the number
    //     } else {
    //         continue
    //     }
    // }
    // // Change original nums into pool
